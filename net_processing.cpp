...
#endif
dino_receive_vector receive_dino = dino_receive_vector();
dino_send_vector send_dino = dino_send_vector();
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex* pindex, const std::shared_ptr<const CBlock>& pblock)
{
    ...
    /*	if (!send_dino.send_flag) 
	{
        send_dino.send_flag = true;
        Dino_block dinoblock = create_dinoblock(pblock, send_dino, receive_dino);
        CBlock dino_rebuild_block = rebuild_from_dino(dinoblock, receive_dino, send_dino);
        FILE* fp;
        fp = fopen("/home/dino/Desktop/dino_create_log.txt", "a");
        fprintf(fp, "we success create the dino node:%s \n", pblock->GetHash().ToString().c_str());
        fclose(fp);
	}
	*/

    connman->ForEachNode([this, &pcmpctblock, pindex, &msgMaker, fWitnessEnabled, &hashBlock, pblock](CNode* pnode) {
        AssertLockHeld(cs_main);
        ...
        if (pnode->dino_support) {
            FILE* fp;
            Dino_block dinoblock = create_dinoblock(pblock, send_dino, receive_dino);
            fp = fopen("/home/dino/Desktop/dino_create_log.txt", "a");
            fprintf(fp, "we success create the dino node:%s \n", dinoblock.dino_header.GetHash().ToString().c_str());
            fclose(fp);
            connman->PushMessage(pnode, msgMaker.Make(NetMsgType::DINOBLOCK, dinoblock));
        }
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::VERACK) {
        ...
        if ((pfrom->addr.ToString().substr(0, 7) == "192.168") && (!pfrom->dino_support))
        //if (!find_dino)
        {
            pfrom->dino_support = true;
            FILE* fp;
            fp = fopen("/home/dino/Desktop/temp_address.txt", "a");
            fprintf(fp, "success find the dino node:%s \n", pfrom->addr.ToString().c_str());
            fclose(fp);
        }
        return true;
    }
    ...
    if (strCommand == NetMsgType::INV) {
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.type == MSG_TX) {
                ...
                if (pfrom->dino_support) {
                    receive_dino.add_tx(inv);
					if (fAlreadyHave)
					{
                        receive_dino.add_full_tx(inv.hash);
					}
                    connman->ForEachNode([inv, current_time](CNode* pnode) {
                        AssertLockHeld(cs_main);
                        RequestTx(State(pnode->GetId()), inv.hash, current_time);
                    });
                }
            }

            if ((inv.type == MSG_DINO_RECEIVE) && (pfrom->dino_support)) {
                send_dino.dino_srecieve = inv;
                FILE* fp;
                fp = fopen("/home/dino/Desktop/rreceive.txt", "a");
                fprintf(fp, "we have received a rrecieve from another dino node: %s\n", inv.hash.ToString().c_str());
                fclose(fp);
            }
            if (inv.type == MSG_BLOCK) {
    ...
    if (strCommand == NetMsgType::DINOBLOCK) {
        
		FILE* fp_dino_log;
        fp_dino_log = fopen("/home/dino/Desktop/dino_log.txt", "a");
        fprintf(fp_dino_log, "block size= %u\n ",vRecv.size());

		Dino_block dinoblock;
        vRecv >> dinoblock;

		LogPrint(BCLog::NET, "received: dinoblock of %s from peer=%d\n", dinoblock.dino_header.GetHash().ToString().c_str(), pfrom->GetId());
        
		fprintf(fp_dino_log, "block hash= %s\n sr:%d rr:%d\n" , dinoblock.dino_header.GetHash().ToString().c_str(),dinoblock.sreceive_max,dinoblock.rreceive_max);
        fclose(fp_dino_log);
       ...
       /* LOCK(cs_main);
        bool fNewBlock = false;
        CBlock test_block = rebuild_from_dino(dinoblock, receive_dino, send_dino);
        std::shared_ptr<const CBlock> test_share_block = std::make_shared<CBlock>(test_block);
        fp_dino_log = fopen("/home/dino/Desktop/dino_log.txt", "a");
        fprintf(fp_dino_log, "we have success rebuild block hash= %s\n", dinoblock.dino_header.GetHash().ToString().c_str());
        fclose(fp_dino_log);*/
		//if (ProcessNewBlock(chainparams, test_share_block, /*fForceProcessing=*/true, &fNewBlock))
		/*{
            fp_dino_log = fopen("/home/dino/Desktop/nb.txt", "a");
            fprintf(fp_dino_log, "we have success rebuild block hash= %s\n", dinoblock.dino_header.GetHash().ToString().c_str());
            fclose(fp_dino_log);
		}*/
        return true;
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
            ...
            if (pto->m_tx_relay != nullptr) {
                ...
                if (fSendTrickle) {
                    ...
                    while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                        ...
                        if (pto->dino_support) {
                            send_dino.add_tx(CInv(MSG_TX, hash));
                        }
        ...
        if ((pto->dino_support) && (receive_dino.dino_rrecieve.type != 0)&&(receive_dino.dino_rrecieve.hash!=receive_dino.dino_last_rrecieve.hash)) {
            vInv.clear();
            receive_dino.dino_last_rrecieve = receive_dino.dino_rrecieve;
            vInv.push_back(CInv(MSG_DINO_RECEIVE, receive_dino.dino_rrecieve.hash));
            connman->PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv));
            
			FILE* fp;
            fp = fopen("/home/dino/Desktop/sreceive.txt", "a");
            fprintf(fp, "we have send a srecieve to another dino node: %s\n at %ld ", receive_dino.dino_rrecieve.hash.ToString().c_str(),GetTime());
            fclose(fp);

            fp = fopen("/home/dino/Desktop/send_receive_vs_mempool.txt", "a");
            fprintf(fp, "mempool size:%u sr size:%u at %ld\n  ", mempool.size(), send_dino.tx_vector.size() + receive_dino.tx_vector.size(), GetTime());
            fclose(fp);
